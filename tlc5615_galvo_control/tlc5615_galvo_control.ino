/*____________SineGen - dual TLC5615__________
 ...
 * - 12-bit base values scaled to 10-bit output
 * - 310-point arrays
 * - Precise trigger timing for synchronization
 * - Non-Blocking mode execution using millis()
 * - Dual TLC5615 (10-bit) on separate CS pins, shared DATA/CLOCK
 */

#include <avr/pgmspace.h>
#define DAC_DATA_PIN 11  // MOSI / TLC5615 DIN
#define DAC_CLOCK_PIN 13 // SCK / TLC5615 SCLK
...
int32_t centreX = 512;  // 10-bit DAC center code
int32_t centreY = 512;
...
void setup() {
  ...
  pinMode(dacY_CS, OUTPUT);
  pinMode(dacX_CS, OUTPUT);
  digitalWrite(DAC_CLOCK_PIN, LOW);
  digitalWrite(dacY_CS, HIGH);
  digitalWrite(dacX_CS, HIGH);
...
void writeDACs(uint16_t dacX_val, uint16_t dacY_val) {
  writeTLC5615(dacY_CS, dacY_val);
  writeTLC5615(dacX_CS, dacX_val);
}

// TLC5615 accepts a 12-bit frame: 10 data bits (MSB first), then two zero bits.
// Data is sampled on rising SCLK edges while CS is low.
void writeTLC5615(byte chipSelect, uint16_t value) {
  uint16_t frame = (value & 0x03FF) << 2;
  digitalWrite(chipSelect, LOW);
  for (int8_t bit = 11; bit >= 0; --bit) {
    digitalWrite(DAC_DATA_PIN, (frame & ((uint16_t)1 << bit)) ? HIGH : LOW);
    digitalWrite(DAC_CLOCK_PIN, HIGH);
    digitalWrite(DAC_CLOCK_PIN, LOW);
  }
...
// Convert the 12-bit waveform around midpoint to a 10-bit TLC5615 code.
uint16_t computeDACValue(uint16_t base, int32_t scale_fixed, int32_t centre) {
  int32_t val = (int32_t)base - 2048;
...
void processSerialCommands() {
  ...
  while(idx < len) {
    ...
    if (cmd == 'c') { enterFixedMode(); idx++; }
      ...
      if (neg) val = -val;
      centreY = constrain(centreY + val, 0, 1023);
      ...
      if (neg) val = -val;
      centreX = constrain(centreX + val, 0, 1023);
...
void printHelp() {
  Serial.println("--- TLC5615 Firmware (10-bit output) ---");
